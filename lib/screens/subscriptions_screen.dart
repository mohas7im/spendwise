...
class _SubscriptionsScreenState extends State<SubscriptionsScreen> {
  ...
  void _showSubscriptionModal(BuildContext context, FinanceProvider provider, {SubscriptionModel? existingSub}) {
    ...
    final colors = [
      Colors.deepPurpleAccent, Colors.redAccent, Colors.green, 
      Colors.blueAccent, Colors.orange, Colors.pink
    ];
    final icons = [
      Icons.subscriptions, Icons.movie, Icons.music_note, 
      Icons.fitness_center, Icons.shopping_cart, Icons.videogame_asset,
      Icons.wifi, Icons.phone_android
    ];
    final currencies = ['₹', '\$', '€', '£'];
        ...
        builder: (context, setModalState) => Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(existingSub == null ? 'Add Subscription' : 'Edit Subscription', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                ...
                Wrap(
                  ...
                  children: icons.map((icon) => GestureDetector(
                    onTap: () => setModalState(() => selectedIcon = icon.codePoint),
                    child: CircleAvatar(
                      backgroundColor: selectedIcon == icon.codePoint ? Color(selectedColor) : Colors.grey.withValues(alpha: 0.2),
                      radius: 20,
                      child: Icon(icon, color: selectedIcon == icon.codePoint ? Colors.white : Colors.grey),
                    ),
                  )).toList(),
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text(existingSub == null ? 'Add Subscription' : 'Update Subscription'),
                  ),
                ),
                const SizedBox(height: 24),
              ],
  ...
  void _showSubscriptionDetails(BuildContext context, SubscriptionModel sub, FinanceProvider provider) {
            ...
            children: [
                ...
                children: [
                  ...
                  PopupMenuButton<String>(
                    ...
                    onSelected: (val) {
                      Navigator.pop(ctx);
                      if (val == 'edit') {
                        _showSubscriptionModal(context, provider, existingSub: sub);
                      } else if (val == 'delete') {
                    ...
                    itemBuilder: (c) => [
                      PopupMenuItem(value: 'pause', child: Text(sub.isPaused ? 'Resume' : 'Pause')),
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      Text('${sub.currency}${sub.cost.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                    ],
                  ),
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                        sub.isPaused ? 'Paused' : '${sub.nextBilling.day}/${sub.nextBilling.month}/${sub.nextBilling.year}',
                        style: TextStyle(
              ...
              if (sub.paymentHistory.isEmpty)
                const Padding(
                  ...
                  child: Text('No actual payments recorded yet. Auto-deductions will appear here.', style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic)),
                )
              else
                ...sub.paymentHistory.reversed.take(5).map((h) => ListTile(
                  contentPadding: EdgeInsets.zero,
                  leading: CircleAvatar(
                    backgroundColor: Colors.green.withValues(alpha: 0.1),
                    child: const Icon(Icons.check, color: Colors.green),
                  ),
                  title: Text('${sub.currency}${h.amount.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text('${h.date.day}/${h.date.month}/${h.date.year} • ${h.status}'),
                )),
            ],
          ),
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final subscriptions = List<SubscriptionModel>.from(financeProvider.subscriptions);
            ...
            children: [
                      ...
                      Row(
                        ...
                        children: [
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('Yearly Equivalent', style: TextStyle(color: subTextColor, fontSize: 11)),
                              Text('₹${(totalYearly + (totalMonthly * 12)).toStringAsFixed(0)}', style: TextStyle(color: textColor, fontWeight: FontWeight.bold, fontSize: 15)),
                            ],
                          ),
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.end,
                            children: [
                              ...
                              Text('${subscriptions.where((s) => !s.isPaused).length}', style: TextStyle(color: textColor, fontWeight: FontWeight.bold, fontSize: 15)),
                            ],
                          ),
                        ],
                      ),
              ...
              Padding(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Your Subs', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold, fontSize: 18)),
                  ...
                  return Opacity(
                    ...
                    child: Card(
                      ...
                      child: InkWell(
                        ...
                        onTap: () => _showSubscriptionDetails(context, sub, financeProvider),
                        child: Padding(
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(sub.isPaused ? 'Paused' : sub.cycle, style: const TextStyle(color: Colors.grey, fontSize: 12)),
                                  ],
                                ),
                              ),
                              Column(
                                crossAxisAlignment: CrossAxisAlignment.end,
                                children: [
                                  Text('${sub.currency}${sub.cost.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                  const SizedBox(height: 4),
                                  Text(
